...
/**
 * Send organization welcome email using Brevo
 */
const sendOrganizationWelcomeEmail = async (adminEmail, adminName, organizationName, loginUrl) => {
  ...
  if (!apiInstance) {
    console.error('❌ Brevo not configured. Email not sent.');
    return false;
  }

  const sendSmtpEmail = new Brevo.SendSmtpEmail();
  
  sendSmtpEmail.subject = `Welcome to FinLight - ${organizationName} Organization Created!`;
  sendSmtpEmail.sender = {
    name: process.env.BREVO_SENDER_NAME || 'FinLight',
  ...
  sendSmtpEmail.htmlContent = `
    ...
    <head>
      ...
      <style>
        ...
        .container { max-width: 600px; margin: 0 auto; padding: 20px; }
        .email-wrapper { background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        ...
        .details { background: #f8f9fa; padding: 20px; border-radius: 12px; margin: 25px 0; border-left: 4px solid #667eea; }
        .button-container { text-align: center; margin: 30px 0; }
        .cta-button { display: inline-block; padding: 14px 35px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; border-radius: 50px; font-weight: 600; font-size: 16px; }
      ...
      <div class="container">
        <div class="email-wrapper">
          ...
          <div class="content">
            ...
            <div class="message">
              Congratulations! Your organization <strong>${organizationName}</strong> has been successfully set up on the FinLight platform.
            </div>
            ...
            <div class="button-container">
              <a href="${loginUrl}" class="cta-button" style="color: white;">Access Your Dashboard →</a>
            </div>
...
const sendPasswordResetEmail = async (email, name, resetUrl) => {
  ...
  sendSmtpEmail.subject = 'Password Reset Request - FinLight';
  sendSmtpEmail.sender = {
    name: process.env.BREVO_SENDER_NAME || 'FinLight',
    email: process.env.BREVO_SENDER_EMAIL || process.env.EMAIL_USER
  };
